...
(defmacro def-status [status-code name]
  (let [status-f-name? (symbol (str name \?))]
    `(do
       (defn ~name
         ([] (~name {}))
         ([request#] (assoc request# :status ~status-code)))
       (defn ~status-f-name? [{status# :status :as response#}]
         (= ~status-code status#)))))

(defmacro def-status-range [start end name]
  (let [status-f-name? (symbol (str name \?))]
    `(defn ~status-f-name? [{status# :status :as response#}]
       (boolean (and status# (<= ~start status# ~end))))))
...
(def-status 451 unavilable-for-legal-reasons)
...
(def-status 507 insufficent-storage)
