...
(defn lru
  ...
  ([] (lru default-lru-threshold default-seed))
  ([seed] (lru default-lru-threshold seed))
  ([threshold seed]
...
(defn- hit-or-miss
  ([^CacheProtocol cache ^clojure.lang.Volatile vres entry]
   (hit-or-miss cache vres entry nil))
...
(defn fcache
  "Creates a generic cache of a function that takes any number of `args`,
  returning the value of applying `args` to `f`.
  Values are remembered according to semantics of the cache `impl`."
  [^CacheProtocol impl f]
  (let [cache (atom impl)]
    (fn [& args]
      (let [vres (volatile! nil)]
        (swap! cache hit-or-miss vres args #(apply f args))
        @vres))))
...
(defn ->SimpleCache
  ...
  and set the value if not found."
  ([^CacheProtocol impl]
   (->SimpleCache impl nil))
